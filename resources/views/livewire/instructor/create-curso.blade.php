    ...
    <form wire:submit.prevent='create' action="">
        <div class="mb-6 grid grid-cols-5 gap-8">
            <div class="col-span-3">
                ...
                <x-input wire:keydown='slugChange' wire:model='title' type="text" class="w-full {{$errors->has('title') ? 'dark:border-red-600' : ''}}" placeholder=''></x-input>
                <x-input-error for='title' />
            </div>
            <div class="col-span-2">
                <x-label for="email">Slug del curso:</x-label>
                <x-input wire:model='slug' readonly type="text"  class="w-full {{$errors->has('slug') ? 'dark:border-red-600' : ''}}" placeholder=''></x-input>
                <x-input-error for='slug' />
            </div>
        </div>
        <div class="mb-6">
            <x-label for="password">Subtítulo del curso:</x-label>
            <x-input wire:model='subtitle' type="text"  class="w-full {{$errors->has('subtitle') ? 'dark:border-red-600' : ''}}" placeholder=''></x-input>
            ...
            <div>
                <div>
                    <label for="dropzone-file"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 {{$errors->has('subtitle') ? 'dark:text-red-600' : 'dark:text-blue-500'}}" fill="none"
                        ...
                        <input id="dropzone-file" type="file" accept="image/*" wire:model="photo" class="hidden" />
                    </label>
                    <x-input-error for='photo' />
                </div>
                <button type="submit"
                    class="text-white  mt-4 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full  px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Guardar
                    información</button>
            </div>
            <div>
                @if ($photo)
